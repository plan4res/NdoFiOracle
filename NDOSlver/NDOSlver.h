...
/** Public enum, meant to be used in (the two overloaded versions of)
    SetPar() [see below] for selecting the algorithmic parameter to be set.
...
/**< Constructor of the class. The parameter `iStrm', if provided, istaken as
   ...
   just to be found in the stream after those of the base class.
    ...
    words, one has to show that there exists a( epsilon-)subgradient of
...
/**< Sets the starting point of the NDO algorithm; if 0 is given, or if
   ...
   (the all-0 vector being one of the prime candidates). Note that Solve()
   [see below] is expected to repotimize somehow using the results of the
   previous calls, if any, and using the latest "current point" as the
   ...
   The vector pointed by tLambda is supposedly copied into internal data
   structures of the NDSolver, hence it can be modified and/or destroyed
...
/**< NDO algorithms typically have some parameters (stepsizes, multipliers
   ...
   that the previous optimization had not been really successful, so that
...
/** Convex minimization problems have a "dual" viewpoint (which is also
    ...
    NDO algorithm should produce a set of multipliers Theta[ z ] attached to
    ...
    In the Lagrangian case, a dual object x[ i ] is attached to the item z with
...
/** Some NDO algorithms may exploit the knowledge of Lower Bounds on the
    ...
    If the NDO algorithm does not exploit the LBs, no dual multipliers are
    associated with them, and this method must return 0. */

   virtual HpNum ReadLBMult( cIndex wFi = Inf< Index >() ) { return( 0 ); }
...
/** Returns the number of times that Fi() has been called; if called from
   ...
   Index FiEval( void ) const { return( FiEvaltns ); }
...
/** Returns the number of times that NewGi() has been called; if called from
   ...
   Index GiEval( void ) const { return( GiEvaltns ); }
...
/** Returns the number of times that Solve() has been called; if called from
   ...
   Index NrCalls( void ) const { return( SCalls ); }
...
/** Returns the current number of iterations. Note that this counter is *not*
   ...
   Index NrIter( void ) const { return( ParIter ); }
...
/** As NDOTime( double & , double & ) [see above], except that returns the
   ...
   double NDOTime( void ) { return( NDOt ? NDOt->Read() : 0 ); }
...
/** Returns the current number of variables of the function; a protected
   ...
   Index GetNumVar( void ) const { return( NumVar ); }
...
/** Adds `NNwVrs' new variables to the NDO problem. The new variables are
    ...
    This implies, for instance, that all the previously collected subgradients
...
/** Removes the variable whose "names" are contained in the vector `whch',
    ...
    *last* variable is eliminated, no renaming has to be done.
    ...
    This implies, for instance, that the projection of all the previously
    ...
    termination of the call to RemoveVariables() the FiOracle must be updated
    ...
    SetNDOSolver() in FiOracle.h]. If RemoveVariables() is not directly
   ...
   virtual void RemoveVariables( cIndex_Set whch = 0 , Index hwmny = 0 ) {
    throw( NDOException(
		   "RemoveVariables: removing variables is not supported" ) );
    }
...
/** This method signals to the NDO solver that there have been changes in
    ...
    the Lagrangian case, changes to the first-order information correspond to
    changes in the constraints `A[ h ]()'/ right hand side `b' (note that
    these changes *are* typically accompanied by changes of the Fi-values).
