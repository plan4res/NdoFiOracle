...
/** The class FiOracle provides a standard interface between NDO solvers and
    ...
    characterized by a real vector in the Lambda-space, will be dealt with
    ...
    subsequent decisions, and that information may have become invalid.
    Pretty often, however, the previous information can be exploited,
    possibly after proper modifications, to "warm-start" the optimization of
    the new function.
...
/** In some cases, an optimal solution of the dual problem (D) [see the
    ...
    eventually to be produced. Also, a "naming protocol" is needed to ensure
...
/** Returns the maximum number of dual information stored in the FiOracle(),
    as set by the latest call to SetMaxName(). A default implementation is
...
/** The function Fi to be minimized may be unbounded below, i.e., its
    ...
    stop, as there is no way of achieving - INF by a finite number of finite
...
/** @defgroup EasyComp Description of "easy" components of Fi()
    ...
    - "easy" components, for which a "compact, efficient" description is
      known.
    ...
    \note *Important*: "easy" components that reveal themselves cannot be
	  ...
	  "easy" the FiOracle is basically freed by all obligations regarding
	  ...
	  FiOracle, which can then avoid to do so if the NDOSolver it's going
    ...
    \note These methods are intended to be called only with 1 <= wFi <=
	  ...
	  are already provided for obtaining this very special description
	  [see GetGi() below].
...
/** Returns the number of variables of the "easy" linear problem which
    ...
    B[ wFi ] and A[ wFi ] and the length of the vectors x[ wFi ], c[ wFi ],
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
/** Returns the number of rows of the matrix B[ wFi ] and the length of the
...
/** The "format" of the vector Lambda set by SetLambda() [see above] depends
    ...
    *nonzero entries* of the vector of variables. There are LmbdBD
    ...
    need to store the pointer (and length) in the corresponding protected data
    structures. */
...
/** The computation of the function Fi may be a costly task, e.g. involving
    ...
    A return value of false, therefore, informs the NDO solver that if
    more accurate values than those previously provided are necessary
    for continuing the optimization process, then it should be stopped
    because such values are not available. A return value of true,
    instead, tells that it is perhaps possible to produce more
    accurate values. In this case, the NDO solver can call again Fi()
    [see below] *without* changing the point to retrieve the
    (hopefully) more accurate values; also, it can ask the FiOracle
    for more (epsilon)-subgradients at the current point, even if it
    had previously declared that it had had enough [see NewGi()
    below]. However, the NDO solver is *not obliged* to verify the
    claim of the FiOracle, that is it can avoid to ask for the
    values/subgradients in the current point.
    ...
    - The first case is when the FiOracle cannot compute the real
      value of Fi, but it is capable of providing a vector that would
      be a 0-subgradient if the value were the one returned. The
      typical case it that of a Lagrangian function where the
      Lagrangian subproblem is not solved to optimality, but the
      suboptimal solution is treated as if it were optimal. In this
      case, a higher accuracy means that better solutions of the
      Lagrangian problem must be obtained, actually providing a
      different (higher) value of the function Fi in the same point
      (and, of course, new subgradients).
...
/** This method must be called to ask the FiOracle whether it can produce a
    ...
    SetLamBase() [see above]. The method can only be called *after* that
...
/**< GetGi() [and GetVal(), see below] can be used to query information
   ...
   which characterizes the item. SubG is a pointer to a vector of SgNum of
   appropriate length (see below) where the item have to be written. The
   ...
   i.e., the length of the "dense" SubG[]. Note that, if SGBse != 0, the
   ...
   of nonzeroes in the item; let us denote that by MxNZ. Then, the length of
...
/** In some cases, a Lower Bound on the minimal value of Fi is known; if Fi
    ...
    although the two can be equal.

    Furthermore, changes in Fi() (number of variables etc.) may possibly
    render any previously obtained Lower Bound invalid, so that this method
    should be called *at least* each time Fi() changes; failure to do that
    could lead to early and incorrect termination of the optimization due to
    the use of an incorrect Lower Bound.

    Lower Bounds may be available separately for the wFi-th component of Fi,
...
/** GetFiStatus() returns the internal status of the FiOracle object. There
    ...
    kFiStop    The meaning of this return value is somehow different for
    ...
    all the above mentioned methods of NDOSolver. In turn, those methods of
...
/** If this method is called within any of the methods of the class that are
    ...
    system time (in seconds) since the start of that method. If methods that
...
/**< If so instructed by SetMaxName() [see above], the FiOracle should keep
   ...
   corresponding to the item named `i' can be discarded; if i > n, then the
