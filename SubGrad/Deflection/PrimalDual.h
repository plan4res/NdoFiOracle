 ...
 *
 * Because this class chooses <em>both</rm> the deflection coefficient and the
...
/* If LOG_VOL > 0, the PrimalDual class produces a log of its activities on
...
/** Constructor of the class, which derives from both Deflection and
   ...
   to be found in the stream \a after those of the base class Stepsize [see
   the comments to the constructor of Stepsize]:
...
/*--------------------------------------------------------------------------*/
  ...
  HpNum GetLev( void )
  {
   throw( NDOException( "PrimalDual::GetLev: not allowed" ) );
   }
...
/*--------------------------------------------------------------------------*/
  ...
  HpNum GetBeta( void )
  {
   throw( NDOException( "PrimalDual::GetBeta: not allowed" ) );
   }
...
/*--------------------------------------------------------------------------*/
  ...
  {
   throw( NDOException( "PrimalDual::SetMaxBeta: not allowed" ) );
   }
...
inline void PrimalDual::NewDEF( void )
{
 if( average )
  ...
  vk = 1 / std::sqrt( Deflection::GetGiNorm() );
...
inline HpNum PrimalDual::GetDFLCoeff( void )
{
 return( vk / Tmpdelta );
 }
...
inline void PrimalDual::UpdateGamma( void )
{
 ...
 if( average )
  gamma = LpsFct * ( std::sqrt( LipCnst / 2.0 )  / tStar );
 else
  gamma = LpsFct / ( std::sqrt( 2.0 * LipCnst ) * tStar );
