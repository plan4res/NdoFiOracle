...
/** @file
 *
 * Implementation of the SubGrad class, which implements the NDOSolverEmptySet
 * interface for NonDifferentiable Optimization Solvers, as described in
 * NDOSlver.h. This class implements several algorithms including
 * deflected, incremental and primal/dual subgradient methods. Furthermore
 * it can use several stepsizes approaches.
 *
...
#include <cmath>
#include <algorithm>  // included to control the random shuffle of the
                      // components of oracle with the incremental approach

/*--------------------------------------------------------------------------*/
/*-------------------------------- MACROS ----------------------------------*/
/*--------------------------------------------------------------------------*/

#define SubGrad_LOG 1

/** If SubGrad_LOG != 0, the SubGrad class produces a log of its activities on
 * the ostream object and at the "level of verbosity" set with the method
 * SetNDOLog() [see below]. */
...
SubGrad::SubGrad( std::istream *iStrm )
         :
         NDOSolver( iStrm )
{
 ...
 stepsize = nullptr;
 deflection = nullptr;
 Lambda = LambdaHat = LambdaBest = nullptr;
 Gi = dir = nullptr;
 ub = lb = nullptr;
...
{
 if( stepsize ) {
  // if the Stepsize and Deflection objects are actually the same, ensure
  // it is not deleted twice
  auto StpDef = dynamic_cast< Deflection * >( stepsize );
  delete stepsize;
  if( ! StpDef )
   deflection = nullptr;
  }
...
void SubGrad::SetDeflection( Deflection *Vol )
{
 if( deflection ) {
  // if the Stepsize and Deflection objects are actually the same, ensure
  // it is not deleted twice
  auto StpDef = dynamic_cast< Stepsize * >( deflection );
  delete deflection;
  if( ! StpDef )
   stepsize = nullptr;
  }
...
void SubGrad::SetFiOracle( FiOracle *Fi )
{
 ...
 if( Fi ) {    // setting a new oracle - - - - - - - - - - - - - - - - - - - -
  ...
  // allocate memory for the subgrads and the direction - - - - - - - - - - -
  //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  // The direction will be allocated after either Deflection will have been
  // defined or changed [see InitDeflection()]. Instead, the subgradient is
  // considered separately from the deflection and it will keep on having the
  // same values after any deflection update.

  Gi = new SgNum[ MaxNumVar ];
  dir = new SgNum[ MaxNumVar ];
  ...
  AllocateUBLB( 0 , NumVar );

  // allocate memory for disjoint simplex constraints - - - - - - - - - - - -
  //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

  #if ! SubGrad_HANDLES_CONSTRAINTS
   if( Oracle->MaxNConst() )
    throw( NDOException( "SubGrad::SetFiOracle: non-box constraints allowed"
			  ) );
  #else
   MaxNCnst = Oracle->MaxNConst();

   if( MaxNCnst ) {
    CnstBeg = new Index[ MaxNCnst ];
    CnstNxt = new SIndex[ MaxNumVar ];
    CnstVol = new HpNum[ MaxNCnst ];
    VectAssign( CnstBeg , Index( Inf< Index >() ) , MaxNCnst );
    VectAssign( CnstNxt , SIndex(-1) , MaxNCnst );
    MaxName = MaxNCnst + 3;
    }
   else
    MaxName = 3;
...
void SubGrad::SetPar( const int wp , const bool value ) {
 switch( wp ) {
  ...
  default:           //- - - - - - - - - - - - - - - - - - - - - - - - - - - -
   throw( NDOException( "SetPar( bool ): unknown parameter" ) );
  }
 }  // end( SubGrad::SetPar( bool ) )

/*--------------------------------------------------------------------------*/

void SubGrad::SetNDOLog( std::ostream *outs , const char lvl )
{
 NDOSolver::SetNDOLog( outs , lvl );

 #if( SubGrad_LOG )
  if( NDOLLvl > 1 ) {
   *NDOLog << std::endl << "SubGrad Solver: Vars = " << NumVar << " ("
	   << MaxNumVar << ") ~  NItIncr = " << NItIncr <<  " ~ EpsLin = "
	   << EpsLin << " ~ FiEps = " << EInit << " ~ t* = " << tStar;

   *NDOLog << std::endl << " SGParameters = ( " << SGPar1 << " ~ " << SGPar2
 ...
 do {
  ...
  if( FiBar < Inf< HpNum >() ) {
   ...
   if( fs == FiOracle::kFiStop ) {
    VLOG( 1 , " ~ FiOracle:STOP" << std::endl );
    Result = kStopped;
    break;
    }

   if( fs == FiOracle::kFiError ) {
    VLOG( 1 , " ~ Error in the FiOracle" << std::endl );
    Result = kError;
    break;
    }
   ...
   if( fs == FiOracle::kFiChgd ) {
    VLOG( 1 , " ~ Fi changed: loop" << std::endl );
    continue;
    }
  ...
  if( InnIter ) {  // if it is the first component, re-shuffle the vector
   ...
   if( ! comp )
    std::shuffle( Seq.begin() , Seq.end() , myrandom );    
   comp = Seq.at( comp );
   }
  else
   comp = Inf< Index >();
  ...
  // check whether or not the Lower Bound has changed - - - - - - - - - - - -
  //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  ...
  if( ( FiBar == Inf< HpNum >() ) || ( ! deflection ) ) {
   ...
   if( deflection ) {
    HpNum Mlt = 0;
    Index NmSt = MaxNCnst;
    Oracle->Aggregate( &Mlt , &NmSt , 1 , MaxNCnst + 2 );
    }
  ...
  if( ( DoSS = deflection->DoSS() ) ) {  // SS- - - - - - - - - - - - - - - -
   ...
   VLOG2( 1 , deflection->Delta() < Inf< HpNum >() , std::endl << " SS["
	      << CSSCntr );
   ...
   VLOG2( 1 , deflection->Delta() > - Inf< HpNum >() , std::endl << " NS["
	      << CNSCntr );
...
void SubGrad::ReSetAlg( unsigned char RstLvl )
{
 ...
 if( ! ( RstLvl & ( RstSbg | RstCnt ) ) ) {  // reset everything - - - - - - -
  ...
  if( ! ( RstLvl & RstSbg ) ) {  // reset the subgradient and the directions
   ...
   for( Index i = MaxName ; i-- > MaxNCnst ; )
    Oracle->Deleted( i );
   }
  #if SubGrad_HANDLES_CONSTRAINTS
   else
    if( ! ( RstLvl & RstCnt ) ) {  // reset the knapsack constraints- - - - -
     for( Index i = MaxNCnst ; i-- ; )
...
bool SubGrad::IsOptimal( HpNum eps )
{
 ...
 if( FiL == Inf< HpNum >() )
  ...
  if( SGPar4 )
   return( tStar * std::sqrt( NrmDir ) + std::min( Epsilon , HatEpsilon )
	   <= eps * FiL );
  else
   return( tStar * std::sqrt( NrmDir ) + Epsilon <= eps * FiL );
  }
...
cHpRow SubGrad::ReadMult( cIndex_Set &I , Index &D , cIndex wFi )
{
 ...
 if( FiBar < Inf< HpNum >() ) {
  if( deflection )
   *MultBse = ( DirPos == first ) ? ( MaxNCnst + 2 ) : ( MaxNCnst + 1 );
  else
   *MultBse = MaxNCnst;
...
void SubGrad::AddVariables( Index NNwVrs , cLMRow IVs )
{
 ...
 #if SubGrad_HANDLES_CONSTRAINTS
  if( MaxNCnst ) {  // the case of knapsack constraints is treated separately
	             //- - - - - - - - - - - - - - - - - - - - - - - - - - - -

   SgRow Cnstr = new SgNum[ NNwVrs ];

   for( Index i = 0 ; i < MaxNCnst ; i++ )
    if( CnstBeg[ i ] < Inf< Index >() ) {
     ...
     if( SGBDim ) { 
      Index_Set SGBse1 = new Index[ NNwVrs + 1 ];
      Index_Set SGBse2;
      if( ! SGBse ) {
 ...
 if( deflection ) {  // if no deflection is done, Dir does not exist - - - - -
  ...
  #if SubGrad_HANDLES_CONSTRAINTS
   if( ( SGPar1 & PrjDIRM1 ) && MaxNCnst )
    strt = 0;
   else
  #endif
    strt = NumVar;

  // DirPos == first => DirM1 is at the position MaxNCnst + 1
  // DirPos == second => DirM1 is at the position MaxNCnst + 2
  //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

  SGBDim = Oracle->GetGi( dir + strt , SGBse , MaxNCnst + ( first ? 1 : 2 ) ,
			  strt , NumVar + NNwVrs );
 ...
 #if SubGrad_HANDLES_CONSTRAINTS
  if( ( SGPar1 & PrjSG ) && MaxNCnst )
   strt = 0;
  else
 #endif
   strt = NumVar;

 SGBDim = Oracle->GetGi( Gi + strt , SGBse , MaxNCnst , strt ,
			 NumVar + NNwVrs );
 if( SGBDim ) {
  if( SGBse )   // if it is given in sparse format, densify it
...
{
 if( whch &&  ( ! hwmny ) )  // removing no variable
  return;                    // cowardly and silently return

 if( whch && ( whch[ hwmny ] != Inf< Index >() ) )
  throw( NDOException( "SubGrad::RemoveVariables: wrong set" ) );
  ...
  if( MaxNCnst ) {
   if( whch ) {
    ...
    for( Index h ; ( h = *(tw++) ) < Inf< Index >() ; ) {
     ...
     Index cnstr = 0;
     bool FoundVar = false;
     for( ; cnstr < MaxNCnst ; cnstr++ ) {
      if( CnstBeg[ cnstr ] < Inf< Index >() ) {
       Index rr = CnstBeg[ cnstr ];
       while(  CnstNxt[ rr ] < MaxNumVar ) {
	if( rr == h ) {
	 FoundVar = true;
	 break;
         }
...
SubGrad::~SubGrad()
{
 ...
 VLOG( 1 , std::endl << "Total Fi() evaluations = " << FiEvaltns );
 VLOG2( 1 , NDOt , std::endl << "Tot. time (s): " << NDOt->Read() );
 VLOG2( 1 , Oracle , std::endl << "Fi() time (s): " << Oracle->FiTime() );
 VLOG( 1 , std::endl );
 ...
 SetStepsize();    // delete Stepsize
 SetDeflection();  // delete Deflection, if any
...
void SubGrad::FiAndGi( cIndex wFi )
{
 ...
 if( FiLambda == Inf< HpNum >() )  // it is a constraint
  #if SubGrad_HANDLES_CONSTRAINTS
   EvalConst();
  #else
   throw( NDOException( "SubGrad::FiAndGi: constraints are not allowed" ) );
  #endif
 else
...
void SubGrad::FormD( void ) {
 ...
 if( SGPar3 & StepRST ) {  // stepsize-restricted approach - - - - - - - - - -
  ...
  if( ( SGPar3 & SafeRULE )  && ! InnIter ) {
   ...
   alpha = std::min( 1.0 , std::max( alpha , target ) );
 ...
 //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 // DirPos == first => DirM1 is at the position MaxNCnst + 1
 // and Dir is registered in the free entry MaxNCnst + 2,
 ...
 if( deflection ) {
  if(  alpha == 1 ) {  // pass the subgradient to the FiOracle object
   ...
   Oracle->Aggregate( &Mlt , &NmSt , 1 ,
		      MaxNCnst + ( DirPos == first ? 2 : 1 ) );
   }
  else {
   HpNum Mlt[ 2 ] = { alpha , 1.0 - alpha };
   if( DirPos == first ) {
    Index NmSt[ 2 ] = { MaxNCnst , MaxNCnst + 1 };
    Oracle->Aggregate( Mlt , NmSt , 2 , MaxNCnst + 2 );
    }
   else {
    Index NmSt[ 2 ] = { MaxNCnst , MaxNCnst + 2 };
    Oracle->Aggregate( Mlt , NmSt , 2 , MaxNCnst + 1 );
    }
   }
  }
...
inline void SubGrad::Log1( void )
{
 #if( SubGrad_LOG )
  if( NDOLLvl > 1 ) {
   *NDOLog << std::endl << "{" << SCalls << "-" << ParIter << "} ";
...
inline void SubGrad::Log4( void )
{
 #if( SubGrad_LOG )
  if( NDOLLvl > 1 ) {
   // if the deflection is used the information relative to the subgradient
   // have already been displayed [see Log2() ]

   *NDOLog << std::endl << "           ";
   if( deflection )
    *NDOLog << " ( |Gi| , Sigma ) = (" << sqrt(NrmGi) << " , " << Sigma
	    << ")";
   }
 #endif
 }  // end( Log4 )
...
void SubGrad::AllocateUBLB( cIndex strt , cIndex num )
{
 LMRow tub = ub ? ub + strt : new LMNum[ num ];
 LMRow tlb = lb ? lb + strt : new LMNum[ num ];
 VectAssign( tlb , - Inf< LMNum >() , num );
 ...
 if( ( ! LBc ) && ( ! UBc ) )
  for( ; i < num ; ++i ) {
   if( ! Oracle->GetUC( strt + i ) ) {
    tlb[ i ] = 0;
    LBc = true;
    }

   if( ( tub[ i ] = Oracle->GetUB( strt + i ) ) < Inf< LMNum >() )
    UBc = true;
 ...
 if( ( ! lb ) && LBc ) {
  if( ( strt == 0 ) && ( num == MaxNumVar ) )
   lb = tlb;
  else {
   lb = new LMNum[ MaxNumVar ];
   cLMNum dflt = - Inf< LMNum >();
   VectAssign( lb , dflt , strt );
   VectAssign( lb + strt , tlb , num );
   delete[] tlb;
   }
  }
 ...
 if( ( ! ub ) && UBc ) {
  if( ( strt == 0 ) && ( num == MaxNumVar ) )
   ...
   ub = new LMNum[ MaxNumVar ];
   cLMNum dflt = Inf< LMNum >();
   VectAssign( ub , dflt , strt );
   VectAssign( ub + strt , tub , num );
   delete[] tub;
   }
  }
 }  // end( AllocateUBLB )
...
bool SubGrad::ProjFsb( LMRow Lpt , bool & EmptySet )
{
 ...
 #if SubGrad_HANDLES_CONSTRAINTS
  if( MaxNCnst ) {  // there are knapsack constraints - - - - - - - - - - - -
                     //- - - - - - - - - - - - - - - - - - - - - - - - - - - -
   if( ! Q2KNP )
    throw( NDOException( "SubGrad::ProjFsb: CQKNP object not present" ) );

   LMRow PjLpt = new LMNum[ NumVar ];  // reset the projected point

   VectAssign( PjLpt , HpNum( Inf< LMNum >() ) , NumVar );
   ...
   for( Index i = 0 ; i < MaxNCnst ; i++ )
    if( CnstBeg[ i ] < Inf< Index >() ) {
     ...
     Row pC = new Number[ Nvar ];  // linear costs
     Row pD = new Number[ Nvar ];  // quadratic costs,
     Row pA = new Number[ Nvar ];  // lower bounds
     Row pB = new Number[ Nvar ];  // upper bounds
     Number pV = CnstVol[ i ];     // volume
 ...
 // project Lpt on the box constraints  - - - - - - - - - - - - - - - - - -

 if( lb && ub ) {
  // meanwhile, check that they are valid
...
void SubGrad::ProjTng( SgRow Gpt , cLMRow Lpt , cIndex strt , Index stp )
{
 ...
 #if SubGrad_HANDLES_CONSTRAINTS
  if( MaxNCnst ) {  // there are some disjoint knapsack constraints - - - - -
                    //- - - - - - - - - - - - - - - - - - - - - - - - - - - -
   if( ! Q2KNP )
    throw( NDOException( "SubGrad::ProjTng: CQKNP object not present" ) );
 
   LMRow PjGpt = new LMNum[ NumVar ];  // reset the projected point
   VectAssign( PjGpt , HpNum( Inf< LMNum >() ) , NumVar );

   for( Index i = 0 ; i < MaxNCnst ; i++ )
    if( CnstBeg[ i ] < Inf< Index >() ) {
     // count the number of variables of the i-th knapsack constraint- - - - -

     Number lhs = 0; // compute the left hand side
     lhs = 0;
     Index Nvar = 0;
     for( Index j = CnstBeg[ i ] ; j == MaxNumVar ; j = CnstNxt[ j ] ) {
      Nvar++;
     ...
     if( ABS( lhs - CnstVol[ i ] ) <= Eps ) {
      ...
      Nvar = 0;
      for( Index j = CnstBeg[ i ] ; j == MaxNumVar + i ; j = CnstNxt[ j ] ) {
       if( LambdaBar[ j ] == ubnd( j ) )
	pB[ Nvar ] = 0;
       else
	pB[ Nvar ] = Inf< Number >();
       if( LambdaBar[ j ] == lbnd( j ) )
	pA[ Nvar ] = 0;
       else
	pA[ Nvar ] = -Inf< Number >();
       pC[ Nvar ] = -2.0 * Gpt[ j ];
	...
	PjGpt[ j ] = *(Gpt1++);
       ...
       throw( NDOException( "SubGrad::ProjTng: error in projection" ) );
      }
 ...
 // project Gpt on the box constraints  - - - - - - - - - - - - - - - - - -
...
HpNum Deflection::GetGiNorm( void )
{
 return( Solver->NrmGi );
 }
...
HpNum Deflection::GetDNorm( void )
{
 return( Solver->NrmDir );
 }
...
HpNum Deflection::GetSigma( void )
{
 return( Solver->Sigma );
 }
...
HpNum Deflection::GetEpsilon( void )
{
...
HpNum Deflection::ReadFVal( void )
{
 return( Solver->FiLambda );
 }
