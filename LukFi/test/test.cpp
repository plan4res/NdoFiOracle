...
/*-------------------------------- main() ----------------------------------*/
/*--------------------------------------------------------------------------*/
/* The parameter file describes both the function to be minimized tha the
 * configuration of the NDOSolver. The format is:
 *
 ...
 *
 * - the four numerical parameters read by the constructor of LukFiOrcl,
 *   the first being which function to minimize (1 .. 25) and the other
 *   three ones extra integer parameters useful for some functions only;
 *   see the comments to the constructor of LukFiOrcl for details
 *
...
int main( int argc , char **argv )
{
 ...
 if( argc < 2 ) {
  cerr << "Usage: " << argv[ 0 ] << " < parfile name >" << endl;
  return( 1 );
  }

 ifstream ParFile( argv[ 1 ] );
 if( ! ParFile.is_open() ) {
  cerr << "Cannot open parameters file """ << argv[ 1 ] << """" << endl;
  return( 1 );
  }
 ...
 try {
  ...
  LukFiOrcl *Fi = new LukFiOrcl( &ParFile );
